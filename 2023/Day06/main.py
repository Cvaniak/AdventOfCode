...
def part_test():
    data = read_data("test")
    assert part_1(data) == 288
    assert part_2(data) == 71503


def part_1(data):
    time = load_ints_seperated(data[0].split()[1:])
    dbg.x(time)
    distances = load_ints_seperated(data[1].split()[1:])
    dbg.x(distances)

    ans = 1
    for t, d in zip(time, distances):
        tmp = 0
        for x in range(t):
            if (x * (t - x)) > d:
                tmp += 1
...
def part_2(data):
    ...
    dbg.x(time, distances)

    z = math.sqrt(time**2 - 4 * distances)
    dbg(z)
    aa = (time - z) / (2)
    bb = (time + z) / (2)
    if aa > bb:
