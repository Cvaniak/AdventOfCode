...
def part_1(data):
    res = 0
    for line in data:
        ans, _, values = line.partition(": ")
        values, ans = list(map(int, values.split(" "))), int(ans)
        if check(ans, values):
            res += ans
    return res
        
def check_2(ans, values):
    n = len(values)
    q = [(values[0], 0)]
    while q:
        curr, idx = q.pop()
        ...
        q.append((curr*values[idx], idx))
        q.append((curr+values[idx], idx))
        q.append((curr*foo(values[idx])+values[idx], idx))
    return False
