...
def calc(value):
    value = (value ^ (value * 64)) % PRUNE
    value = (value ^ (value // 32)) % PRUNE
    value = (value ^ (value * 2048)) % PRUNE
    return value


def part_test():
    data = read_data("test")
    assert part_1(data) == 37327623
...
def part_2(data):
    ...
    for x in data:
        ...
        for _ in range(1995):
            ...
            curr = x % 10
            a, b, c, d = b, c, d, curr - prev
            if (a, b, c, d) not in this_one:
                counter[(a, b, c, d)] += curr
                this_one.add((a, b, c, d))
            prev = curr
