...
def part_test():
    ...
    assert part_2(data) == "co,de,ka,ta"
...
def part_1(data):
    ...
    groups = set()
    mp_list = list(mp.keys())
    for idx_a in range(len(mp_list)):
        a = mp_list[idx_a]
        for idx_b in range(idx_a+1, len(mp_list)):
            b = mp_list[idx_b]
            if a not in mp[b]:
                continue
            for w in mp[a] & mp[b]:
                if any(x in contain_t for x in [a, b, w]):
                    groups.add(tuple(sorted([a, b, w])))
    return len(groups)
...
def part_2(data):
    ...
    for idx_a in range(len(mp_list)):
        ...
        for b in mp[a]:
            ...
            groups[tuple(sorted(cc))] += 1

    for xx in groups.most_common():
        x = xx[0]
        if len(x)*(len(x)-1) == xx[1]:
            return ",".join(x)
