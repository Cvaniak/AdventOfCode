...
def part_test():
    ...
    assert part_2(data) == 43


def single_step(matrix):
    taken = []
    for y in range(matrix.n):
        for x in range(matrix.m):
            if matrix.s_get(y, x) == ".":
                continue
            count = sum([int(matrix.s_get(y+dy, x+dx)=="@") for dy, dx in dir_8])
            if count < 4:
                taken.append((y, x))
...
def part_2(data):
    matrix = Matrix(data)
    answer = 0
    while new_added:=single_step(matrix):
        answer += new_added
    return answer
