...
def get_data(data):
    ...
    for line in data:
        ...
        if first:
            ...
            ranges.append(Data(int(a), int(b)))
        else:
...
def part_2(data):
    ranges, _ = get_data(data)
    ranges.sort()
    union = [ranges[0]]
    for r in ranges[1:]:
        if union[-1].b >= r.b:
            continue
        if union[-1].b >= r.a and union[-1].b < r.b:
            union[-1] = Data(union[-1].a, r.b)
            continue
        union.append(r)
    return sum(b-a+1 for a, b in union)
