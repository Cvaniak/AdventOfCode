...
def load_int_lines(data):
    ...
    for idx, i in enumerate(data):
        data[idx] = [int(x) for x in i.split()]
    return data
...
def part_test():
    data = read_data("test")
    assert part_1(data) == 4277556
    assert part_2(data) == 3263827


def part_1(data):
    int_lines = load_int_lines(data[:-1])
    signs = data[-1].split()
    answer = 0
    d = {"+": sum, "*": prod}
    for i in range(len(signs)):
        answer += d[signs[i]]([int_lines[x][i] for x in range(len(int_lines))])
    return answer


def part_2(data):
    int_strings = list(zip(*data[:-1]))[::-1]
    int_lines = ["".join(x) for x in int_strings]
    n = len(int_lines)
    signs = data[-1].split()[::-1]
    d = {"+": sum, "*": prod}
    answer = 0
    idx = 0
    for sign in signs:
