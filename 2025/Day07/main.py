...
def part_test():
    ...
    assert part_2(data) == 40


def number_of_splits(data):
    answer = 0
    pos = {data[0].index("S")}
    for line in data[1:]:
        new_pos = pos.copy()
...
def number_of_timelines(data):
    pos = defaultdict(int)
    pos[data[0].index("S")] = 1
    for line in data[1:]:
        new_pos = pos.copy()
        for idx, c in enumerate(line):
            if c == "^" and idx in pos:
                new_pos[idx - 1] += pos[idx]
                new_pos[idx + 1] += pos[idx]
                new_pos[idx] = 0
        pos = new_pos
...
def part_1(data):
    return number_of_splits(data)


def part_2(data):
    return number_of_timelines(data)
