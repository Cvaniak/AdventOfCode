...
def part_test():
    data = read_data("test")
    #assert part_1(data) == 2 <- This is real task for this day
    assert part_2(data) == "This is last STAR!"
...
def load(data):
    ...
    for line in data:
        ...
        if line == "":
            a = 0
            for row in rows:
                a += sum(int(x) for x in row)
            shapes.append(Shape(rows, a))
            continue
        if ":" in line:
            size, _, nums = line.partition(": ")
            x, _, y = size.partition("x")
            x, y = int(x), int(y)
            regions.append(Region(x, y, load_ints_seperated(nums, " ")))
            continue
        rows.append([True if x == "#" else False for x in line])
    return shapes, regions


def easy_fit(region, shapes):
    return region.x*region.y >= sum(region.req)*9


def hard_fit(region, shapes):
    return region.x*region.y >= sum(shapes[idx].num*num for idx, num in enumerate(region.req))


def part_1(data):
    shapes, regions = load(data)
    answer = 0
    for r in regions:
        es = easy_fit(r, shapes)
        hd = hard_fit(r, shapes)
        if es is True and hd is True:
            answer += 1
        if es != hd:
            assert False
